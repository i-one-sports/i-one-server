...
import { CreateTournamentDto, RegisterTeamDto, UpdateMatchResultDto, UpdateTournamentDto } from "./dto/tournament.dto";
import { CustomHttpException, Group, Team, Tournament, TournamentFormat, TournamentMatch, TournamentStatus } from "@app/common";
...
export class TournamentsService {
    ...
    const organizer = await this.userRepository.findOne(
        { _id: createTournamentDto.organizer});
    if (!organizer) {
      throw new CustomHttpException('Organizer user not found', HttpStatus.NOT_FOUND);
    }

    const existingTournament = await this.tournamentRepository.findOne({ tag: createTournamentDto.tag });
    if (existingTournament) {
      throw new CustomHttpException(`Tournament with tag ${createTournamentDto.tag} already exists`, HttpStatus.CONFLICT);
    }
  ...
  async findOne(id: string): Promise<Tournament> {
    ...
    if (!tournament) {
      throw new CustomHttpException(`Tournament with ID ${id} not found`, HttpStatus.NOT_FOUND);
    }
    return tournament;
  }

  async findByTag(tag: string): Promise<Tournament> {
    ...
    if (!tournament) {
      throw new CustomHttpException(`Tournament with tag ${tag} not found`, HttpStatus.NOT_FOUND);
    }
  ...
  async update(id: string, updateTournamentDto: UpdateTournamentDto): Promise<Tournament> {
    ...
    if (updateTournamentDto.tag && updateTournamentDto.tag !== tournament.tag) {
      ...
      if (existingTournament) {
        throw new CustomHttpException(`Tournament with tag ${updateTournamentDto.tag} already exists`, HttpStatus.CONFLICT);
      }
    }
  ...
  async registerTeam(tournamentId: string, registerTeamDto: RegisterTeamDto): Promise<Tournament> {
    const tournament: Tournament = await this.tournamentRepository.findOne({
        _id: tournamentId
    });
    if (!tournament) {
      throw new CustomHttpException(`Tournament with ID ${tournamentId} not found`, HttpStatus.NOT_FOUND);
    }
    ...
    if (!team) {
      throw new CustomHttpException(`Team with ID ${registerTeamDto.teamId} not found`, HttpStatus.NOT_FOUND);
    }

    // Check if team is already registered
    if (tournament.registeredTeams.some(teamId => teamId.toString() === registerTeamDto.teamId)) {
      throw new CustomHttpException('Team is already registered for this tournament', HttpStatus.BAD_REQUEST);
    }

    return await this.tournamentRepository.findRaw().findByIdAndUpdate(
        tournamentId,
  ...
  async unregisterTeam(tournamentId: string, teamId: string): Promise<Tournament> {
    ...
    if (!tournament.registeredTeams.some(registeredTeamId => registeredTeamId.toString() === teamId)) {
      throw new CustomHttpException('Team is not registered for this tournament', HttpStatus.BAD_REQUEST);
    }

    // Remove team from tournament
    tournament.registeredTeams = tournament.registeredTeams.filter(registeredTeamId => 
      registeredTeamId.toString() !== teamId
    );
    
    return this.tournamentRepository.findRaw().findByIdAndUpdate(
      tournamentId,
  ...
  async startTournament(tournamentId: string): Promise<Tournament> {
    ...
    if (tournament.format === TournamentFormat.UCL_CLASSIC) {
      if (tournament.registeredTeams.length < 16) {
        throw new CustomHttpException('Tournament needs at least 16 teams to start', HttpStatus.BAD_REQUEST);
      }
      if (tournament.registeredTeams.length > 32) {
        throw new CustomHttpException('Tournament can have at most 32 teams', HttpStatus.BAD_REQUEST);
      }
    }
    ...
    switch (tournament.format) {
      case TournamentFormat.UCL_CLASSIC:
        ...
        break;
      default:
        throw new CustomHttpException(`Tournament format ${tournament.format} not supported yet`, HttpStatus.BAD_REQUEST);
    }
  ...
  private async setupUCLClassicFormat(tournament: Tournament, teams: Team[]): Promise<void> {
      ...
      for (let i = 0; i < group.teams.length; i++) {
        for (let j = i + 1; j < group.teams.length; j++) {
          ...
          const homeMatch: TournamentMatch = {
            ...
            scheduledDate: new Date(now.getTime() + (matches.length * 24 * 60 * 60 * 1000)), // 1 day apart
            completed: false,
            stage: 'group_stage',
            group: group.name
          };
          ...
          const awayMatch: TournamentMatch = {
            ...
            scheduledDate: new Date(now.getTime() + ((matches.length + 1) * 24 * 60 * 60 * 1000)), // 1 day apart
            completed: false,
            stage: 'group_stage',
            group: group.name
          };
  ...
  async updateMatchResult(tournamentId: string, matchIndex: number, updateMatchResultDto: UpdateMatchResultDto): Promise<Tournament> {
    const tournament = await this.tournamentRepository.findOne({
      _id: tournamentId
    });
    if (!tournament) {
    ...
    if (matchIndex < 0 || matchIndex >= tournament.matches.length) {
      throw new CustomHttpException(`Match index ${matchIndex} not found in tournament`, HttpStatus.NOT_FOUND);
    }
    ...
    if (match.completed) {
      throw new CustomHttpException('Match is already completed', HttpStatus.BAD_REQUEST);
    }
  ...
  private async updateGroupStandings(tournament: Tournament, match: TournamentMatch): Promise<void> {
    // Find the group
  ...
  private async processKnockoutMatchResult(tournament: Tournament, matchIndex: number): Promise<void> {
    const match = tournament.matches[matchIndex];
    let winner: string;
  ...
  private addWinnerToQuarterFinals(tournament: Tournament, winnerTeamId: string, matchIndex: number): void {
    ...
    const existingMatchIndex = tournament.matches.findIndex(m => 
      m.stage === 'quarter_final' && m.homeTeam === null && m.awayTeam === null
    );
  ...
  private addWinnerToSemiFinals(tournament: Tournament, winnerTeamId: string, matchIndex: number): void {
  ...
  private addWinnerToFinal(tournament: Tournament, winnerTeamId: string, matchIndex: number): void {
  ...
  private async checkAndAdvanceTournamentStage(tournament: Tournament): Promise<void> {
    ...
    if (groupStageCompleted && tournament.status === TournamentStatus.GROUP_STAGE) {
  ...
  async getTournamentsByOrganizerId(organizerId: string): Promise<Tournament[]> {
  ...
  async getUpcomingMatches(tournamentId: string, teamId?: string): Promise<TournamentMatch[]> {
    const tournament = await this.tournamentRepository.findOne({
      _id: tournamentId
    });
  ...
  async getCompletedMatches(tournamentId: string, teamId?: string): Promise<TournamentMatch[]> {
    const tournament = await this.tournamentRepository.findOne({
      _id: tournamentId
  });
    if (!tournament) {
  ...
  async getGroupStandings(tournamentId: string, groupName?: string): Promise<Group[]> {
    const tournament = await this.tournamentRepository.findOne({
      _id: tournamentId
    });
    ...
    if (groupName) {
      ...
      if (!group) {
        throw new CustomHttpException(`Group ${groupName} not found in tournament`, HttpStatus.NOT_FOUND);
      }
      return [group];
    }
