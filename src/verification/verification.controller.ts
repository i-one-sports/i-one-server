...
import { CurrentUser, User, UploadType } from '@app/common';
...
export class VerificationController {
  ...
  async submitVerificationDocuments(
    @UploadedFiles() files: { frontPage?: Express.Multer.File[], backPage?: Express.Multer.File[] },
  ...
  async getMyVerification(@CurrentUser() user: User) {
    return this.verificationService.getVerificationByUserId(user._id.toString());
  }
  ...
  async rejectVerification(@Param('id') id: string) {
    return this.verificationService.rejectVerification(id);
  }
}
